    ...
    if (v4l2_config[i].video_buf.video_fd == -1) {
      ...
      if (nc_v4l2_dequeue_buffer(v4l2_config[i].video_buf.video_fd,
        ...
        if (v4l2_config[i].data_idx == OFRC_MODE) {
          glTexImage2D(
              GL_TEXTURE_2D, 0, GL_RGBA, v4l2_config[i].sensor_width / 2,
              v4l2_config[i].sensor_height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
              v4l2_config[i].video_buf.buffers[video_buf.index].start);
        } else {
#if 0 // Legacy: Original RGB rendering
          glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, VIDEO_WIDTH, VIDEO_HEIGHT, 0,
                       GL_RGB, GL_UNSIGNED_BYTE,
                       v4l2_config[i].video_buf.buffers[video_buf.index].start);
#else
          IplImage *src_img = cvCreateImageHeader(
              cvSize(VIDEO_WIDTH, VIDEO_HEIGHT), IPL_DEPTH_8U, 3);
          src_img->imageData =
              (char *)v4l2_config[i].video_buf.buffers[video_buf.index].start;

          static IplImage *gray_img = NULL;
          if (gray_img == NULL) {
            gray_img = cvCreateImage(cvSize(VIDEO_WIDTH, VIDEO_HEIGHT),
                                     IPL_DEPTH_8U, 1);
          }

          cvCvtColor(src_img, gray_img, CV_RGB2GRAY);

          glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, VIDEO_WIDTH,
                       VIDEO_HEIGHT, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE,
                       gray_img->imageData);
...
static void signal_int() { running = 0; }
