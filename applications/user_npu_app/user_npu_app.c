...
/*    don't modify  */
#define MAX_WIDTH_FOR_VDMA_CNN_DS (3840)
#define MAX_HEIGHT_FOR_VDMA_CNN_DS (2160)
/********************/
...
static int dsr_init(void) {
  ...
  dsr_input_config.format =
      IMG_FORMAT_YUV422; // Changed from RGB888 for 4K YUYV support
  dsr_input_config.width = MAX_WIDTH_FOR_VDMA_CNN_DS;
  dsr_input_config.height = MAX_HEIGHT_FOR_VDMA_CNN_DS;
  dsr_output_config.format = IMG_FORMAT_RGB888; // DSR output to NPU remains RGB

  BUFF_SIZE = dsr_input_config.width * dsr_input_config.height *
              2; // YUYV is 2 bytes per pixel
...
#endif
        {
          ...
          memcpy(dsr_info.dsr_in_buf[0],
                 ...
                 MAX_WIDTH_FOR_VDMA_CNN_DS * MAX_HEIGHT_FOR_VDMA_CNN_DS *
                     2); // YUYV is 2 bytes per pixel (replaced RGB_CNT)
