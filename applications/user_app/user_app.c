...
static volatile int running = 1; // Mark as volatile for signal handling
...
void render(void *data, struct wl_callback *callback, uint32_t time_val) {
  ...
  for (int i = 0; i < VIDEO_MAX_CH; i++) {
    if (v4l2_config[i].video_buf.video_fd == -1) {
      ...
      if (nc_v4l2_dequeue_buffer(v4l2_config[i].video_buf.video_fd,
        ...
        // Unified rendering logic for both OFRC_MODE and others
        int width = v4l2_config[i].sensor_width;
        int height = v4l2_config[i].sensor_height;

        // Use the raw buffer start pointer
        char *raw_data =
            (char *)v4l2_config[i].video_buf.buffers[video_buf.index].start;

        if (v4l2_config[i].data_idx == OFRC_MODE) {
          // OFRC mode usually provides YUYV directly in this setup
          // We need to convert it to RGB to draw colors

          // Create Header for YUYV data
          IplImage *yuyv_img =
              cvCreateImageHeader(cvSize(width, height), IPL_DEPTH_8U, 2);
          yuyv_img->imageData = raw_data;

          // Create/Reuse RGB image
          static IplImage *rgb_img = NULL;
          static int last_width = 0, last_height = 0;

          if (rgb_img == NULL || last_width != width || last_height != height) {
            if (rgb_img)
          ...
          // Convert YUYV to RGB
          cvCvtColor(yuyv_img, rgb_img, CV_YUV2RGB_YUYV);
          ...
          int cx = width / 2;
          int cy = height / 2;
          int radius = (height < width ? height : width) / 5;

          // Draw Clock Face
          cvCircle(rgb_img, cvPoint(cx, cy), radius, cvScalar(255, 255, 255, 0),
                   2, 8, 0);
          cvCircle(rgb_img, cvPoint(cx, cy), 5, cvScalar(0, 0, 255, 0), -1, 8,
                   0);

          // Calculate Angles
          float sec_angle = timeinfo->tm_sec * 6 * (M_PI / 180.0);
          float min_angle = timeinfo->tm_min * 6 * (M_PI / 180.0);
          float hour_angle =
              (timeinfo->tm_hour % 12 * 30 + timeinfo->tm_min * 0.5) *
              (M_PI / 180.0);

          // Draw Hands
          cvLine(rgb_img, cvPoint(cx, cy),
                 cvPoint(cx + (int)(radius * 0.9 * sin(sec_angle)),
                         cy - (int)(radius * 0.9 * cos(sec_angle))),
                 cvScalar(0, 0, 255, 0), 2, 8, 0);

          cvLine(rgb_img, cvPoint(cx, cy),
                 cvPoint(cx + (int)(radius * 0.8 * sin(min_angle)),
                         cy - (int)(radius * 0.8 * cos(min_angle))),
                 cvScalar(0, 255, 0, 0), 4, 8, 0);

          cvLine(rgb_img, cvPoint(cx, cy),
                 cvPoint(cx + (int)(radius * 0.5 * sin(hour_angle)),
                         cy - (int)(radius * 0.5 * cos(hour_angle))),
                 cvScalar(255, 0, 0, 0), 6, 8, 0);

          // Upload RGB texture
          glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB,
                       GL_UNSIGNED_BYTE, rgb_img->imageData);

          cvReleaseImageHeader(&yuyv_img);

        } else {
          // Fallback or other modes (using previous logic if needed, but for
          // now we focus on fixing OFRC) Previously this block handled the
          // clock, but since we are in OFRC mode now, we can duplicate or
          // unify. Given the user is in OFRC mode, the above block is what
          // matters. We will leave a minimal fallback here matching the
          // original structure but updated.

          // ... (This part was the one running before)
          // For safety, let's just use the same logic if it's not OFRC but has
          // valid buffer
  ...
  sprintf(buftext, "GL: %lums, Frame: %lums/%lufps", opengl_time, frametime,
          fpscount_00);
...
static void signal_int() {
  running = 0;
  // Note: async-signal-safe functions only in handlers usually, but printf
  // helpful for debug here If strict safety needed, avoid printf. But for this
  // hang debug, it's fine.
}
