...
#ifdef USE_REMOVED_TRACK_LIMITED
    if (this->removed_stracks_.size() > 100) {
                                                     ...
                                                     std::vector<STrackPtr> &b_res) const
{
    ...
    for (int i = 0; i < overlapping_combinations.size(); i++)
    {
        int a_idx = overlapping_combinations[i].first;
        int b_idx = overlapping_combinations[i].second;
