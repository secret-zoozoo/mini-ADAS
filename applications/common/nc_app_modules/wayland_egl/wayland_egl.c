...
void(*fptr) (void*, wl_callback*, unsigned int);

typedef void (*wayland_egl_t)(void *, struct wl_callback*, unsigned int);
...
void nc_wayland_display_draw(window *pwindow, void *redraw)
{
    struct wl_region *region;

    struct wl_callback_listener frame_listener = {(wayland_egl_t)redraw};

    if (pwindow->opaque || pwindow->fullscreen) {
        region = wl_compositor_create_region(pwindow->display->compositor);
        wl_region_add(region, 0, 0,
                  pwindow->geometry.width,
                  pwindow->geometry.height);
        wl_surface_set_opaque_region(pwindow->surface, region);
        wl_region_destroy(region);
    } else {
        wl_surface_set_opaque_region(pwindow->surface, NULL);
    }

    pwindow->callback = wl_surface_frame(pwindow->surface);
    wl_callback_add_listener(pwindow->callback, &frame_listener, pwindow);

    eglSwapBuffers(pwindow->display->egl.dpy, pwindow->egl_surface);
}

void nc_wayland_display_init(display *pdisplay, void *redraw)
{
    fptr = (wayland_egl_t)redraw;
    ...
    wl_registry_add_listener(pdisplay->registry, &registry_listener, pdisplay);

    wl_display_dispatch(pdisplay->display);

    init_egl(pdisplay, pdisplay->window->opaque);
    create_surface(pdisplay->window);

    pdisplay->cursor_surface =
        wl_compositor_create_surface(pdisplay->compositor);
}

int nc_wayland_display_destroy(display *pdisplay)
{
    destroy_surface(pdisplay->window);
    fini_egl(pdisplay);

    wl_surface_destroy(pdisplay->cursor_surface);
    if (pdisplay->cursor_theme)
        wl_cursor_theme_destroy(pdisplay->cursor_theme);

    if (pdisplay->shell)
        wl_shell_destroy(pdisplay->shell);
