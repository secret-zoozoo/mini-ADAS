...
int nc_v4l2_close(int fd) { return close(fd); }
...
int nc_v4l2_set_format(int fd, unsigned int width, unsigned int height,
...
/*
 *  The operation must be performed in the order of `VIDIOC_S_FMT ->
 * VIDIOC_S_SELECTION`.
 */
int nc_v4l2_set_vdma_cnn_crop_ds(int fd, char dma_mode,
                                 unsigned int img_process, unsigned int pos_x,
                                 unsigned int pos_y, unsigned int crop_width,
  ...
  user_env[idx++] =
      0; // Interleave Order = 0:RGB 1: RBG 2: GRB 3:GBR 4:BRG 5:BGR 6/7: RGB
...
int nc_v4l2_start_capture(int fd) {
  ...
  if (ioctl(fd, VIDIOC_STREAMON, &type) < 0) {
    perror("nc_v4l2_start_capture: VIDIOC_STREAMON failed");
    printf("errno: %d\n", errno);
    return -1;
  }
  return 0;
...
void nc_vdma_memory_dump_image(int ch, const void *p) {
  ...
  if (snprintf(FileName, 256, "./Ch%02d_frame_size_%dx%d_%04d.raw", ch, width,
               height, FrameCnt) >= 256) {
...
int nc_v4l2_get_format(int fd, int *width, int *height) {
  ...
  *height = format.fmt.pix.height;
  printf("[DEBUG] nc_v4l2_get_format: width=%d, height=%d, pixelformat=0x%08X "
         "(%c%c%c%c)\n",
...
int nc_v4l2_init_device_and_stream_on(st_nc_v4l2_config *s_nc_config,
  ...
  ret = nc_v4l2_get_format(s_nc_config->video_buf.video_fd, &sensor_width,
                           &sensor_height);
  if (ret < 0) {
    printf("nc_v4l2_get_format() failure!\n");
  }
  printf("[app] detected pixel format: fourcc(%d) aka %c%c%c%c\n",
         s_nc_config->pixformat, s_nc_config->pixformat & 0xFF,
         (s_nc_config->pixformat >> 8) & 0xFF,
         (s_nc_config->pixformat >> 16) & 0xFF,
         (s_nc_config->pixformat >> 24) & 0xFF);
  ...
  // sensor_height);
  plane_cnt = nc_v4l2_set_format(s_nc_config->video_buf.video_fd, sensor_width,
                                 sensor_height, s_nc_config->pixformat);
  if (plane_cnt < 0) {
  ...
  if (s_nc_config->data_idx == CNN_MODE) {
    ret = nc_v4l2_set_vdma_cnn_crop_ds(
        s_nc_config->video_buf.video_fd, s_nc_config->dma_mode,
        s_nc_config->img_process, s_nc_config->crop_x_start,
        s_nc_config->crop_y_start, s_nc_config->crop_width,
        s_nc_config->crop_height, s_nc_config->ds_width,
    ...
    ret = nc_v4l2_set_vdma_coda_crop_ds(
        s_nc_config->video_buf.video_fd, s_nc_config->img_process,
        s_nc_config->crop_x_start, s_nc_config->crop_y_start,
        s_nc_config->crop_width, s_nc_config->crop_height,
        s_nc_config->ds_width, s_nc_config->ds_height);
  ...
  s_nc_config->video_buf.buffers = (vbuffer *)calloc(
      video_buffer_num, sizeof(*s_nc_config->video_buf.buffers));
  if (!s_nc_config->video_buf.buffers) {
    printf("calloc() failure\n");
    return -1;
  }

  ret = nc_v4l2_alloc_mapped(s_nc_config->video_buf.video_fd, video_buffer_num,
                             s_nc_config->video_buf.buffers);
  if (ret < 0) {
...
void nc_v4l2_show_user_config(st_nc_v4l2_config *s_nc_config, int video_max) {
  ...
  char field_name[10][20] = {
      {"data_idx"},     {"dma_mode"},     {"img_process"}, {"pixformat"},
      {"crop_x_start"}, {"crop_y_start"}, {"crop_width"},  {"crop_height"},
      {"ds_width"},     {"ds_height"},
  };

  char data_idx_string[4][5] = {{"OFRC"}, {"CNN "}, {"CODA"}, {"CSI "}};
    ...
    for (j = 0; j < video_max; j++) {
      ...
      case 0:
        sprintf(sprintf_buffer, "\t%s\t",
                data_idx_string[(s_nc_config[j].data_idx)]);
        printf("%s", sprintf_buffer);
        break;
      case 1:
        if (s_nc_config[j].data_idx == CNN_MODE) {
          sprintf(sprintf_buffer, "\t%s",
                  dma_mode_string[(unsigned char)s_nc_config[j].dma_mode]);
        } else {
          sprintf(sprintf_buffer, "\t%s\t", "-");
        }
        printf("%s", sprintf_buffer);
      ...
      case 2:
        sprintf(sprintf_buffer, "\t%s",
                img_process_string[s_nc_config[j].img_process]);
        printf("%s", sprintf_buffer);
        break;
      case 3:
        ...
        break;
      case 4:
        if (s_nc_config[j].img_process == MODE_DS ||
            s_nc_config[j].img_process == MODE_BYPASS) {
          sprintf(sprintf_buffer, "\t%s\t", "-");
        } else {
      ...
      case 7:
        ...
        break;
      case 8:
        if (s_nc_config[j].img_process == MODE_CROP ||
            s_nc_config[j].img_process == MODE_BYPASS) {
          sprintf(sprintf_buffer, "\t%s\t", "-");
        } else {
...
int nc_v4l2_isp_ctrl(int fd, unsigned char func_num, unsigned char cfg_value) {
  struct v4l2_ext_controls user_ext_controls;
  struct v4l2_ext_control user_control;
  unsigned char user_env[DEFAULT_BUFFER_NUM] = {
      0,
  };
  int idx = 0;
