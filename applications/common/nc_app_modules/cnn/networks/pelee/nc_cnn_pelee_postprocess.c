...
/**
********************************************************************************
* @file    : nc_cnn_pelee_postprocess.c
*
* @brief   : nc_cnn_pelee_postprocess source
*
...
stNetwork_info* g_peleedet_net_info = NULL;

#ifdef USE_SYNC_OBJ_DET_AND_GPIO_OUT
#define MAX_TARGET_ID_CNT 3
static void nc_set_gpio_detect_obj(stCnnPostprocessingResults *get_value)
{
    unsigned int obj_cnt_total=0;
    int target_id[MAX_TARGET_ID_CNT]={4};  //write target id

    for(int i=0;i<PELEE_MAX_CLASS_ID_CNT;i++)
    {
        int flag = 0;

        for(int j=0; j < (sizeof(target_id)/sizeof(int)); j++)
        {
              if(i==target_id[j])
              {
                flag = 1;
        ...
        if(flag==1) obj_cnt_total += get_value->class_objs[i].obj_cnt;
    }

    if( obj_cnt_total > 0) {
        nc_gpio_set(GPIOA, 13);
    } else {
        nc_gpio_clr(GPIOA, 13);
    }
...
/////////////////////////////////////////////////////////////////////////////////
//                                                                             //
//                   for process and draw pelee inference result               //
//                                                                             //
/////////////////////////////////////////////////////////////////////////////////
static void nc_pelee_detections(detection *dets, int *det_num, unsigned int target_w, unsigned int target_h, cnn_output_info *cnn_output)
{
    ...
    int boxes_num = g_peleedet_net_info->anchor_num;//PELEE_ANCHORS_NUM;

    const int screen_size = gridside_width * gridside_height;
    const int box_size = (5 + g_peleedet_net_info->class_num) * screen_size;


    float *ppf32anchors = (float*)g_peleedet_net_info->anchor_size;
    ...
    for (side_y = 0; side_y < gridside_height; ++side_y) {
        for (side_x = 0; side_x < gridside_width; ++side_x) {
            for (obj_index = 0; obj_index < boxes_num; ++obj_index)
            {
                ...
                dets[*det_num].objectness = (confidence > (float)g_peleedet_net_info->obj_th) ? confidence : 0;

                if (dets[*det_num].objectness>0)
                {
                    float x = (nc_Logistic_activate(output_tiled_data[box_index + 0 * screen_size]) + (float)side_x) * (float)cell_size_x;
                    float y = (nc_Logistic_activate(output_tiled_data[box_index + 1 * screen_size]) + (float)side_y) * (float)cell_size_y;// + offest;
                    float w = ppf32anchors[obj_index] * exp(output_tiled_data[box_index + 2 * screen_size]) * (float)cell_size_x;
                    float h = ppf32anchors[obj_index + boxes_num] * exp(output_tiled_data[box_index + 3 * screen_size]) * (float)cell_size_y;
                    ...
                    for (int class_index = 0; class_index < g_peleedet_net_info->class_num; ++class_index) {
                        ...
                        dets[*det_num].prob[class_index] = (curr_score > (float)g_peleedet_net_info->det_th) ? curr_score : 0;
#if 0
...
static void nc_pelee_store_postprocess_results(detection *dets, int *det_num, stCnnPostprocessingResults *out_cnn_results)
{
    int i,j =0;
    ...
    for (i = 0; i < *det_num; i++)
    {
        ...
        float best_score = 0;
        int class_index = 0;
        for (class_index = 0; class_index < g_peleedet_net_info->class_num; class_index++) {
        ...
        for (j = 0; j < g_peleedet_net_info->class_num; j++)
        {
            if ((pred_class == j) & (dets[i].objectness*best_score >= (float)g_peleedet_net_info->det_th))
            {
...
static void nc_postprocess_pelee_detections(unsigned int target_w, unsigned int target_h, cnn_output_info *cnn_output, stCnnPostprocessingResults *out_cnn_results)
{
    // static uint64_t framecnt = 0;
    detection dets[BOX_MAX_NUM];
    int det_num = 0;
    ...
    nc_pelee_detections(dets, &det_num, target_w, target_h, cnn_output);
    ...
    if((cnn_output->index_of_total+1) == cnn_output->total_tensor_cnt)
    {
...
#endif
        nc_Nms_box(dets, &det_num, g_peleedet_net_info->class_num, (float)g_peleedet_net_info->nms_th);
#ifdef SHOW_NMS_TIMEMEASURE
        ...
        nc_pelee_store_postprocess_results(dets, &det_num, out_cnn_results);
#ifdef USE_BYTETRACK
        // Execute tracker
...
void postprocess_pelee_detections(unsigned int canvas_w, unsigned int canvas_h, cnn_output_info *cnn_output)
{
    ...
    static stCnnPostprocessingResults *pelee_detect_results = NULL;
    if (cnn_output->index_of_total == 0) {
        ...
        pelee_detect_results = &(pp_buf->cnn_result);
    }
    #ifdef USE_SYNC_OBJ_DET_AND_GPIO_OUT
    nc_set_gpio_detect_obj(&pelee_detect_results);
    #endif
    nc_postprocess_pelee_detections(canvas_w, canvas_h, cnn_output, pelee_detect_results);
    ...
    if ((cnn_output->index_of_total+1) == cnn_output->total_tensor_cnt && pp_buf != NULL) {
        ...
        pp_buf->draw_info.max_class_cnt = g_peleedet_net_info->class_num;
        pp_buf->draw_info.class_names = g_peleedet_net_info->class_name;
        pp_buf->draw_info.class_colors = (stRGB24*)g_peleedet_net_info->class_color;
...
int nc_postprocess_pelee_inference_result(unsigned int canvas_w, unsigned int canvas_h, struct inference_result_msg *inference_res)
{
    int ret = 0;

    if(inference_res->network_id != NETWORK_PELEE_DET) {
        return -1;
    }

    g_peleedet_net_info = nc_cnn_get_network_info(inference_res->network_id);
    if (g_peleedet_net_info == NULL){
        printf("%s, nc_cnn_get_network_info fail. net_id:%d\n", __FUNCTION__, inference_res->network_id);
        return -1;
    }

    postprocess_pelee_detections(canvas_w, canvas_h, &(inference_res->cnn_output));
    return ret;
}
