...
#define STREAM_BUF_SIZE_AV1                 (40*1024*1024)  // max bitstream size(HEVC:10MB,VP9:not specified)
#define STREAM_BUF_SIZE_AVS2                (40*1024*1024)  // max bitstream size()
...
static void InjectErrorEvery188(BSFeeder bsf, void* data, Uint32 size, void* arg __attribute__((unused)))
{
...
static CNMComponentParamRet GetParameterFeeder(ComponentImpl* from __attribute__((unused)), ComponentImpl* com, GetParameterCMD commandType, void* data)
{
...
static CNMComponentParamRet SetParameterFeeder(ComponentImpl* from __attribute__((unused)), ComponentImpl* com, SetParameterCMD commandType, void* data)
{
...
static BOOL PrepareFeeder(ComponentImpl* com, BOOL *done)
{
    ...
    bsBuffer = (vpu_buffer_t*)osal_malloc((Int32)(com->numSinkPortQueue*sizeof(vpu_buffer_t)));
    for (i=0; i<ctx->numBuffers; i++) {
        bsBuffer[i].size = (Uint32)config->bsSize;
...
static BOOL ExecuteFeeder(ComponentImpl* com, PortContainer* in __attribute__((unused)), PortContainer* out)
{
    FeederContext*          ctx       = (FeederContext*)com->context;
...
static Component CreateFeeder(ComponentImpl* com, CNMComponentConfig* comConfig)
{
    ...
    if (config->bsSize == 0) {
        if(PRODUCT_ID_W_SERIES(config->productId)) {
            ...
            case STD_AV1 : config->bsSize = STREAM_BUF_SIZE_AV1; break;
            case STD_VP9 : config->bsSize = STREAM_BUF_SIZE_VP9; break;
            case STD_AVS2 : config->bsSize = STREAM_BUF_SIZE_AVS2; break;
            default: config->bsSize = STREAM_BUF_SIZE_HEVC;
            }
        } else {
