...
static void PrintH265UserData(Uint8* pBase, Uint32 iFlags, vpu_buffer_t* vb __attribute__((unused)))
{
...
static void PrintH264UserData(Uint8* pBase, Uint32 iFlags, vpu_buffer_t* vb __attribute__((unused)))
{
    user_data_entry_t*  pEntry = NULL;
    user_data_entry_t*  item   = NULL;
...
static void PrintUserData(DecListenerContext* ctx __attribute__((unused)), CNMComListenerDecDone*  lsnpDecDone)
{
...
void HandleDecCompleteSeqEvent(Component com __attribute__((unused)), CNMComListenerDecCompleteSeq* param, DecListenerContext* ctx)
{
...
void HandleDecRegisterFbEvent(Component com __attribute__((unused)), CNMComListenerDecRegisterFb* param, DecListenerContext* ctx)
{
    Uint32 fps = (ctx->fps == 0) ? 30 : ctx->fps;
    //SimpleRenderer_SetFrameRate(ctx->renderer, 0);
    if ( ctx->bandwidth != 0 ) {
        ctx->bwCtx = (BWCtx*)BWMonitorSetup(param->handle, TRUE, (ctx->bandwidth-1), GetBasename((const char *)ctx->inputPath));
    }
    if ( ctx->performance ) {
...
void HandleDecInterruptEvent(Component com __attribute__((unused)), CNMComListenerDecInt* param, DecListenerContext* ctx)
{
...
void HandleDecGetOutputEvent(Component com __attribute__((unused)), CNMComListenerDecDone* param, DecListenerContext* ctx)
{
    ...
    memset(&rcDisplay, 0, sizeof(VpuRect));
    ...
    if (ctx->compareType == YUV_COMPARE) {
        rcDisplay.right = param->output->rcDisplay.right;
        rcDisplay.left  = param->output->rcDisplay.left;
        rcDisplay.top   = param->output->rcDisplay.top;
        rcDisplay.bottom = param->output->rcDisplay.bottom;
        if (ctx->enableScaler == TRUE) {
...
void HandleDecCloseEvent(Component com __attribute__((unused)), CNMComListenerDecClose* param __attribute__((unused)), DecListenerContext* ctx)
{
    if (ctx->bwCtx != NULL)
