...
BOOL MD5Comparator_Compare(
    ComparatorImpl* impl,
    void*           data,
    PhysicalAddress size __attribute__((unused)),
    Uint32          insNum
    )
{
    md5CompContext*    ctx = (md5CompContext*)impl->context;
    ...
    if ( data == (void *)COMPARATOR_SKIP ) {
        for (idx=0; idx<ctx->md5Size; idx++) {
            ...
            if (IsEndOfFile((FILE*)ctx->fp) == TRUE) {
                impl->eof = TRUE;
                break;
            }
        };
    ...
    do {
        ...
        if (impl->usePrevDataOneTime == TRUE) {
            ...
            osal_memcpy(md5, ctx->prevMd5, (Int32)(ctx->md5Size*sizeof(md5[0])));
        }
        else {
            for (idx=0; idx<ctx->md5Size; idx++) {
    ...
    osal_memcpy(ctx->prevMd5, md5, (Int32)(ctx->md5Size*sizeof(md5[0])));
