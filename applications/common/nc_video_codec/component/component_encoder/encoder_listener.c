...
void HandleEncHandlingIntEvent(Component com __attribute__((unused)), CNMComListenerHandlingInt* param __attribute__((unused)), EncListenerContext* ctx)
{
    if (ctx->bwCtx != NULL) {
...
void HandleEncFullEvent(Component com __attribute__((unused)), CNMComListenerEncFull* param __attribute__((unused)), EncListenerContext* ctx __attribute__((unused)))
{
#ifdef SUPPORT_READ_BITSTREAM_IN_ENCODER
#define STREAM_READ_CNT 3
...
#ifdef SUPPORT_READ_BITSTREAM_IN_ENCODER
    if (TRUE == PRODUCT_ID_W_SERIES(productID) && absReader->fp ) {
...
void HandleEncGetEncCloseEvent(Component com __attribute__((unused)), CNMComListenerEncClose* param __attribute__((unused)), EncListenerContext* ctx)
{
    if (ctx->pfCtx != NULL) {
...
void HandleEncCompleteSeqEvent(Component com __attribute__((unused)), CNMComListenerEncCompleteSeq* param, EncListenerContext* ctx)
{
    ...
    if (ctx->bandwidth != 0) {
        ctx->bwCtx = (void**)BWMonitorSetup(param->handle, TRUE, (ctx->bandwidth-1),GetBasename((const char *)ctx->cfgFileName));
    }
}

void HandleEncGetOutputEvent(Component com __attribute__((unused)), CNMComListenerEncDone* param, EncListenerContext* ctx)
{
    ...
    if (output->bitstreamSize > 0) {
        ...
        if (ctx->es || param->fp) {
            ...
            if ( BitstreamReader_Act(param->bsReader, 0, 0, compSize, ctx->es) == FALSE )
                ctx->match = FALSE;
        }
#endif
...
void Coda9HandleEncGetOutputEvent(Component com __attribute__((unused)), CNMComListenerEncDone* param, EncListenerContext* ctx)
{
    EncOutputInfo* output = param->output;
    EncHandle      handle = param->handle;
...
void Coda9HandleEncMakeHeaderEvent(Component com __attribute__((unused)), CNMComListenerEncMakeHeader* param, EncListenerContext* ctx)
{
