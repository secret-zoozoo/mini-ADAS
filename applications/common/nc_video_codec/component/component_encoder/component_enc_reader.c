...
static CNMComponentParamRet GetParameterReader(ComponentImpl* from __attribute__((unused)), ComponentImpl* com, GetParameterCMD commandType, void* data)
{
...
static CNMComponentParamRet SetParameterReader(ComponentImpl* from __attribute__((unused)), ComponentImpl* com __attribute__((unused)), SetParameterCMD commandType, void* data __attribute__((unused)))
{
    BOOL result = TRUE;
...
static BOOL ExecuteReader(ComponentImpl* com, PortContainer* in, PortContainer* out __attribute__((unused)))
{
    ReaderContext*          ctx     = (ReaderContext*)com->context;
            ...
            if ( ctx->ringBuffer == TRUE) {
                ...
                if (ctx->bsReader != 0) {
                    ...
                    if ( 0 != srcData->buf.phys_addr) {//for sequence change
                        if ( FALSE == BitstreamReader_Act(ctx->bsReader, 0, 0, srcData->size, ctx->es) ) {
                            success = FALSE;
                        }
                    }
...
static BOOL PrepareReader(ComponentImpl* com, BOOL* done)
{
    ...
    *done = FALSE;
    ...
    bsBuffer = (vpu_buffer_t*)osal_malloc((Int32)(num * sizeof(vpu_buffer_t)));
    for (i = 0; i < num; i++) {
