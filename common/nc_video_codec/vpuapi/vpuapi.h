...
#define WAVE5_DEC_AV1_MVCOL_BUF_SIZE(_w, _h)        (((VPU_ALIGN64(_w)/64)*256 + (VPU_ALIGN256(_w)/64)*128)*(VPU_ALIGN64(_h)/64) + ((VPU_ALIGN64(_w)/64)*(VPU_ALIGN64(_h)/64)*512) + 41984 + 8192 + 4864)
...
typedef enum {
    ...
    DEC_PIC_W_THUMBNAIL = 0x10, /**< It is thumbnail mode (skip non-IRAP without reference reg.) */
...
#define VPU_ALIGN1024(_x)           (((_x)+0x3ff)&~0x3ff)
...
*/
    ...
    ENC_CONFIG_SUB_FRAME_SYNC,         /**< Set EncSubFrameSyncConfig structure. (CODA9 only)  */
    ENC_SET_SUB_FRAME_SYNC,            /**< write SUB_FRAME_SYNC_IF register.*/
/**
@verbatim
...
*/
    ...
    ENC_SET_SEI_NAL_DATA, /**< This command sets variables in <<vpuapi_h_HevcSEIDataEnc>> structure for SEI encoding.  */
...
/**
 * @brief   This is an enumeration type for representing H.264/AVC decoder error conceal modes. (coda9 only)
 */
typedef enum {
...
/**
 * @brief   This is an enumeration type for representing error conceal modes. (wave only)
 */
typedef enum {
    ERROR_CONCEAL_MODE_OFF         = 0, /**< Conceal off  */
    ERROR_CONCEAL_MODE_INTRA_ONLY  = 1, /**< Intra conceal in intra-picture, inter-picture */
    ERROR_CONCEAL_MODE_INTRA_INTER = 2  /**< Intra conceal in intra-picture, Inter conceal in inter-picture */
} ErrorConcealMode;

/**
 * @brief   This is an enumeration type for representing error conceal unit. (wave only)
 */
typedef enum {
...
typedef struct {
    ...
    BOOL    supportVcpuBackbone;        /**< This indicates whether a product supports the vcpu backbone version. */
    BOOL    supportUserQMatrix;         /**< This indicates whether a product supports the user q matrix table. */
    BOOL    supportWeightedPrediction;  /**< This indicates whether a product supports the weighted prediction. */
    BOOL    supportAdaptiveRounding;    /**< This indicates whether a product supports the adaptive rounding. */
    BOOL    supportRDOQ;                /**< This indicates whether a product supports the RDOQ block. */
} VpuAttr;
...
typedef struct {
    ...
    Uint32          bufYSize;   /**< It indicates the size for Y component in the physical address space when linear map is used. It is the size for Y component when tiled map is used (CODA9). It is also compressed Y buffer size or ARM compressed framebuffer size (WAVE). */
    Uint32          bufCbSize;  /**< It indicates the size for Cb component in the physical address space when linear map is used. It is the size for Cb component when tiled map is used (CODA9). It is also compressed CbCr buffer size (WAVE). */
    Uint32          bufCrSize;  /**< It indicates the size for Cr component in the physical address space when linear map is used. It is the size for Cr component when tiled map is used (CODA9). */
/**
@verbatim
...
/**
@verbatim
It sets the unit of error conceal.

@* 0: Picture-level error conceal
@* 1: Slice/Tile-level error conceal
@* 2: Block-Row-level error conceal
@* 3: Block-level error conceal
@endverbatim
*/
    ErrorConcealUnit errorConcealUnit;
/**
@verbatim
It sets the mode of error conceal.

@* 0: Conceal off
@* 1: Intra conceal in intra-picture, inter-picture
@* 2: Intra conceal in intra-picture, Inter conceal in inter-picture
@endverbatim
*/
    ErrorConcealMode errorConcealMode;
} DecOpenParam;
...
*/
    ...
    BOOL    disableFilmGrain; /**< It disables film grain regardless of syntax value named "apply_grain". (AV1 only) */
} DecParam;
...
*/
typedef struct {
    int allowScreenContentTools; /**< It indicates whether screen content tool is enabled. */
    int allowIntraBC;            /**< It indicates whether intra block copy is enabled. */
} Av1Info;
...
/**
* @brief This is an enumeration for encoder parameter change. (CODA9 encoder only)
*/
typedef enum {
    ENC_CHANGE_PARAM_GOP_NUM       = (1<<0),
    ENC_CHANGE_PARAM_INTRA_QP      = (1<<1),
    ENC_CHANGE_PARAM_BIT_RATE      = (1<<2),
    ENC_CHANGE_PARAM_FRAME_RATE    = (1<<3),
    ENC_CHANGE_PARAM_INTRA_REFRESH = (1<<4),
    ENC_CHANGE_PARAM_SLICE_MODE    = (1<<5),
    ENC_CHANGE_PARAM_MIN_MAX_QP    = (1<<6),
    ENC_CHANGE_PARAM_HEC_MODE      = (1<<6),
    ENC_CHANGE_PARAM_CABAC_MODE    = (1<<7),
    ENC_CHANGE_PARAM_PPS_ID        = (1<<11),
    ENC_CHANGE_PARAM_PIC_PARAM     = (1<<14)
} Coda9ChangeParam;
...
typedef struct {
    ...
    int useMultiRefP;                   /**< A flag to use multi reference picture for P picture. It is valid only if PIC_TYPE is P */
...
image::../figure/wave5xx_ctumap.svg["Format of custom Map", width=300]
...
@* 0 : Disables a user scaling list.
@* 1 : Enables a user scaling list.
@* 2 : Enable a default scaling list.
*/
...
typedef enum {
    ...
    ENC_SET_CHANGE_PARAM_RC_FRAME_RATE       = (1<<6),
...
*/
    ...
    // ENC_SET_CHANGE_PARAM_RC_FRAME_RATE
    int frameRate;                      /**< A frame rate indicator for rate control */
    ...
    */
    ...
    // ENC_SET_CHANGE_PARAM_VUI_HRD_PARAM
    Uint32 encodeHrdRbspInVPS;        /**< It encodes the HRD syntax rbsp into VPS. */
...
/**
* @brief    This is a data structure for declaring SubFrameSync mode and interface in encoder. (CODA9 only)

*/
typedef struct {
/**
@verbatim
@* 0 : Wire-based SubFrameSync
@* 1 : Register-based SubFrameSync
@endverbatim
*/
    Uint32 subFrameSyncMode;
/**
@verbatim
This is a flag to turn on/off the SubFrameSync feature.

@* 0 : disable SubFrameSync.
@* 1 : enable SubFrameSync.
@endverbatim
*/
    Uint32 subFrameSyncOn;
    Uint32 sourceBufNumber;         /**< The number of source frame buffer. Set 1 to this variable if there is no buffering. */
    Uint32 sourceBufIndexBase;      /**< A GDI index (FrameBuffer.myIndex) of source frame buffer for SubFrameSync */
    Uint32 curEncSourceIdx;         /**< An index of source buffer where source frame is currently being encoded in multiple buffering environment.*/
} EncSubFrameSyncConfig;

/**
* @brief This is a data structure for representing the status of Subframe Sync(SFS) when SFS works in register-based mode.
*/
typedef struct {
    Uint32 ipuEndOfRow;   /**< This field notifies that 64 lines has been written into source frame buffer. Encoder starts encoding whenever 64 lines are filled and this field is toggled. */
    Uint32 ipuNewFrame;   /**< In multiple buffering environment, this field notifies that another source frame buffer starts to fill a new frame. It is toggled whenever a new frame starts to be loaded.  */
/**
@verbatim
An index of buffer where source frame is currently being written in multiple buffering environment. It is allowed to set max. 5 buffers for register-based SFS and max. 3 buffer for wire-based SFS.

@* 1: Source frame buffer 0 (Set 1 if no use of multiple buffering, which is default.)
@* 2: Source frame buffer 1
@* 4: Source frame buffer 2
@* 8: Source frame buffer 3
@* 16: Source frame buffer 4
@endverbatim
*/
    Uint32 ipuCurFrameIndex;
    int ipuCurRowIdx;   /**< This variable indicates the number of 64-line row that has been encoding. HOST only uses this variable which does not affect actual hardware behavior. */
} EncSubFrameSyncState;

#ifdef RC_MIN_MAX_PARA_CHANGE
/**
* @brief    This data structure is used when HOST wants to change min/max QP values.
*/
typedef struct {
    int maxQpIEnable; /**< A maximum QP enable flag for I picture */
    int maxQpI;       /**< A maximum QP of I picture for rate control */
    int minQpIEnable; /**< A minimum QP enable flag for I picture */
    int minQpI;       /**< A minimum QP of I picture for rate control */
    int maxQpPEnable; /**< A maximum QP enable flag for P picture */
    int maxQpP;       /**< A maximum QP of P picture for rate control */
    int minQpPEnable; /**< A minimum QP enable flag for P picture */
    int minQpP;       /**< A minimum QP of P picture for rate control */
} MinMaxQpChangeParam;
#endif
...
/**
* @brief    This data structure is used when HOST wants to change picture level parameter values.
*/
typedef struct{
    int MaxdeltaQp; /**< A maximum delta QP for HVS */
    int MindeltaQp; /**< A minimum delta QP for HVS */
...
#ifdef RC_MIN_MAX_PARA_CHANGE
    MinMaxQpChangeParam minMaxQpParam;  /**< <<vpuapi_h_MinMaxQpChangeParam>> for Min/Max Qp change */
#endif
#ifdef RC_PIC_PARACHANGE
    ChangePicParam changePicParam; /**< <<vpuapi_h_ChangePicParam>> */
#endif
} ParamChange;
...
*/
    ...
    int             subFrameSyncEnable;     /**< It enable subframe synchronization for the instance. (WAVE5 only) */
    int             subFrameSyncMode;       /**< It sets <<vpuapi_h_SubFrameSyncMode>>. (WAVE5 only) */
    ...
    Uint32          encodeVuiRbsp;      /**< A flag to encode the VUI syntax rbsp in rbsp which is given by HOST application */
    Uint32          vuiRbspDataSize;    /**< The bit size of the VUI rbsp data */
    PhysicalAddress vuiRbspDataAddr;    /**< The address of the VUI rbsp data */
} EncOpenParam;
...
*/
    ...
    int             forcePicType;       /**< A force picture type (I, P, B, IDR). It is valid when forcePicTypeEnable is 1. (WAVE only) */
...
*/
 typedef struct {
    ...
    BOOL    encodeAUD;          /**< A flag to encode AUD nal unit explicitly (WAVE only) */
} EncHeaderParam;
