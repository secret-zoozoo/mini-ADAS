...
/* #define DISPLAY_ENC_PROCESSING_TIME */
...
static void SetEncPicParam(ComponentImpl* com, PortContainerYuv* in, EncParam* encParam)
{
    ...
    if ((Uint32)testEncConfig.forceIdrPicIdx == frameIdx) {
    ...
    if (frameIdx == (Uint32)(testEncConfig.outNum - 1)) { // last frame
    ...
    if (in->prevMapReuse == FALSE) {
        ...
        if ( (testEncConfig.seiDataEnc.prefixSeiNalEnable || testEncConfig.seiDataEnc.suffixSeiNalEnable) && encParam->srcEndFlag != 1) {
            if (testEncConfig.prefix_sei_nal_file_name[0] && testEncConfig.seiDataEnc.prefixSeiNalEnable) {
                ...
                vdi_write_memory(testEncConfig.coreIdx, ctx->vbPrefixSeiNal[encParam->srcIdx].phys_addr, pUserBuf, testEncConfig.seiDataEnc.prefixSeiDataSize, testEncConfig.stream_endian);
                osal_free(pUserBuf);
            }
            ...
            if (testEncConfig.suffix_sei_nal_file_name[0] && testEncConfig.seiDataEnc.suffixSeiNalEnable) {
                ...
                vdi_write_memory(testEncConfig.coreIdx, ctx->vbSuffixSeiNal[encParam->srcIdx].phys_addr, pUserBuf, testEncConfig.seiDataEnc.suffixSeiDataSize,  testEncConfig.stream_endian);
                osal_free(pUserBuf);
            }
...
static BOOL RegisterFrameBuffers(ComponentImpl* com)
{
    ...
    if (ctx->testEncConfig.seiDataEnc.prefixSeiNalEnable) {
        if (ctx->testEncConfig.prefix_sei_nal_file_name[0]) {
            ...
            if ((ctx->testEncConfig.prefix_sei_nal_fp = (FILE*)osal_fopen(ctx->testEncConfig.prefix_sei_nal_file_name, "r")) == NULL) {
                VLOG(ERR, "fail to open Prefix SEI NAL Data file, %s\n", ctx->testEncConfig.prefix_sei_nal_file_name);
                return FALSE;
            }
        }

        for (idx = 0; idx < (Uint32)srcFbAllocInfo.num ; idx++) {         // the number of roi buffer should be the same as source buffer num.
    ...
    if (ctx->testEncConfig.seiDataEnc.suffixSeiNalEnable) {
        if (ctx->testEncConfig.suffix_sei_nal_file_name[0]) {
            ...
            if ((ctx->testEncConfig.suffix_sei_nal_fp = (FILE*)osal_fopen(ctx->testEncConfig.suffix_sei_nal_file_name, "r")) == NULL) {
...
static CNMComponentParamRet GetParameterEncoder(ComponentImpl* dummy __attribute__((unused)), ComponentImpl* com, GetParameterCMD commandType, void* data)
{
    EncoderContext*             ctx = (EncoderContext*)com->context;
    BOOL                        result  = TRUE;
    ParamEncNeedFrameBufferNum* fbCount;
...
static CNMComponentParamRet SetParameterEncoder(ComponentImpl* dummy __attribute__((unused)), ComponentImpl* com, SetParameterCMD commandType, void* data)
{
    BOOL result = TRUE;
...
static BOOL SetSequenceInfo(ComponentImpl* com)
{
    ...
    if ( (Uint32)ctx->encOpenParam.sourceBufCount > ctx->fbCount.srcFbNum)
...
static BOOL EncodeHeader(ComponentImpl* com)
{
    ...
    ctx->stateDoing = TRUE;
    buf = (vpu_buffer_t*)Queue_Dequeue(ctx->encOutQ);
...
#ifdef DISPLAY_ENC_PROCESSING_TIME
...
static BOOL Encode(ComponentImpl* com, PortContainerYuv* in, PortContainerES* out)
{
    ...
    CNMComListenerEncDone   lsnpPicDone;
    CNMComListenerEncReadyOneFrame  lsnpReadyOneFrame;

    memset(&lsnpPicDone, 0, sizeof(CNMComListenerEncDone));
    memset(&lsnpReadyOneFrame, 0, sizeof(CNMComListenerEncReadyOneFrame));
    ...
    if (((Uint32)ctx->testEncConfig.numChangeParam > ctx->changedCount) &&
        ((Uint32)ctx->testEncConfig.changeParam[ctx->changedCount].setParaChgFrmNum == ctx->frameIdx)) {
        doChangeParam = TRUE;
    }
    ...
    if (doEncode == TRUE) {
        ...
        else if ( lsn.firstQueueFullFlag == TRUE) {
            ctx->firstQueueFullFlag = lsn.firstQueueFullFlag;
        }
#endif
    }
#ifdef SUPPORT_TESTCASE_CQ_16
    else if ( ctx->firstQueueFullFlag == FALSE) {
        return TRUE; /* Try again */
    }
#endif
    ...
    for (i = 0; (Uint32)i < ctx->fbCount.srcFbNum; i++) {
...
static BOOL OpenEncoder(ComponentImpl* com)
{
    ...
    //VPU_EncGiveCommand(ctx->handle, ENABLE_LOGGING, 0); // for debug: print all sw register of VPU
    ...
    *done = FALSE;
    ...
    if (ctx->encOpenParam.ringBufferEnable ==  TRUE) {
        ...
        for (i=0; i<com->numSinkPortQueue; i++) {
            if ( i < ctx->bsBuf.num) {
                ...
                ctx->bsBuffer[i] = (vpu_buffer_t*)osal_malloc(sizeof(vpu_buffer_t));
    ...
    if (ctx->encOpenParam.EncStdParam.waveParam.scalingListEnable) {
        if (testEncConfig->scaling_list_fileName) {
            ...
            if ((testEncConfig->scaling_list_file = (FILE*)osal_fopen(testEncConfig->scaling_list_fileName, "r")) == NULL) {
    ...
    if (ctx->encOpenParam.EncStdParam.waveParam.customLambdaEnable) {
        if (testEncConfig->custom_lambda_fileName) {
            ...
            if ((testEncConfig->custom_lambda_file = (FILE*)osal_fopen(testEncConfig->custom_lambda_fileName, "r")) == NULL) {
    ...
    if (testEncConfig->roi_enable) {
        if (testEncConfig->roi_file_name) {
            ...
            if ((testEncConfig->roi_file = (FILE*)osal_fopen(testEncConfig->roi_file_name, "r")) == NULL) {
    ...
    if (testEncConfig->lambda_map_enable) {
        if (testEncConfig->lambda_map_fileName) {
            ...
            if ((testEncConfig->lambda_map_file = (FILE*)osal_fopen(testEncConfig->lambda_map_fileName, "r")) == NULL) {
    ...
    if (testEncConfig->mode_map_flag) {
        if (testEncConfig->mode_map_fileName) {
            ...
            if ((testEncConfig->mode_map_file = (FILE*)osal_fopen(testEncConfig->mode_map_fileName, "r")) == NULL) {
    ...
    if (testEncConfig->wp_param_flag & 0x1) {
        if (testEncConfig->wp_param_fileName) {
            ...
            if ((testEncConfig->wp_param_file = (FILE*)osal_fopen(testEncConfig->wp_param_fileName, "r")) == NULL) {
...
#ifdef SUPPORT_READ_BITSTREAM_IN_ENCODER
    VLOG(TRACE, "SUPPORT_READ_BITSTREAM_IN_ENCODER\n");
